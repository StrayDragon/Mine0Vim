	...
	if candidate then
		...
		if not vim.g._provider_python_warned then
			...
			vim.schedule(function()
				vim.notify(
					string.format(
						"[nvim] 未找到 Python provider。请在 %s 下创建并安装: python3 -m venv .venv && %s/.venv/bin/pip install -U pip pynvim",
						cfg,
						cfg
					),
					vim.log.levels.WARN
	...
	if candidate then
		...
		if not vim.g._provider_node_warned then
			...
			vim.schedule(function()
				vim.notify(
					string.format(
						"[nvim] 未找到 Node provider。请在 %s 下安装: npm i neovim 或 pnpm add neovim",
						cfg
					),
					vim.log.levels.WARN
				)
