...
-- 工作区符号（所有语言统一 - 基础版本）
map('n', '<leader>ws', vim.lsp.buf.workspace_symbol, vim.tbl_extend('force', opts, { desc = '工作区符号 (基础)' }))
...
-- 复制所有内容到系统剪贴板
map('n', '<Leader>Y', [[ggVG"+y<CR><C-o><C-o>]], vim.tbl_extend('force', opts, { desc = "复制所有内容到系统剪贴板" }))

-- 窗口分割（<leader>S 分组）
map('n', '<Leader>SL', ':set splitright<CR>:vsplit<CR>', vim.tbl_extend('force', opts, { desc = "右侧垂直分割" }))
map('n', '<Leader>SH', ':set nosplitright<CR>:vsplit<CR>', vim.tbl_extend('force', opts, { desc = "左侧垂直分割" }))
map('n', '<Leader>SK', ':set nosplitbelow<CR>:split<CR>', vim.tbl_extend('force', opts, { desc = "上方水平分割" }))
