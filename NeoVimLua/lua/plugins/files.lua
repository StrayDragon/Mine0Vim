	...
	-- Modern file management with nvim-tree.lua (fast and lightweight)
	{
		...
		config = function(_, opts)
			...
			map(
				"n",
				"<A-1>",
				nvim_tree_open_smart,
				vim.tbl_extend("force", opts_noremap, { desc = "Nvim-tree file browser (smart toggle)" })
			)
			...
			map(
				...
				vim.tbl_extend("force", opts_noremap, { desc = "Nvim-tree find current file" })
			)
